...
Please visit your account page on [circleci.com](https://circleci.com/account/api) and to set it in the \`CIRCLECI_API_TOKEN\` environment variable on your CI environment.`,
  ...
  ENOCIRCLECICLI: () => ({
    ...
    details: `The \`circleci\` command line has to be installed in your CI environment and available in the \`PATH\` environment variable.

See [CircleCI installation](${linkify('README.md#circleci-installation')}) for more details.`,
  }),
  ENOORBNAME: () => ({
    message: 'Missing `orbName` option.',
    details: `Please make sure to add a valid \`orbName\` option when configuring semantic release. It should be in the format \`organization/orb\`.`,
  }),
  EINVALIDORBNAME: () => ({
